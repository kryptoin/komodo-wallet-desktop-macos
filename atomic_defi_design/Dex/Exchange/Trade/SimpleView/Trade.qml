...
ClipRRect // Trade Card
{
    ...
    readonly property var sell_ticker_balance: parseFloat(API.app.portfolio_pg.portfolio_mdl.coin_balance(left_ticker))
        ...
        function onBetterOrderDetected(newOrder)
        {
            ...
            if (Constants.API.app.trading_pg.selected_order_status == SelectedOrderStatus.BetterPriceAvailable)
            {
                ...
                if (parseFloat(newOrder.base_max_volume) <= selectedOrder.base_max_volume && parseFloat(newOrder.rel_max_volume) >= Constants.API.app.trading_pg.total_amount)
                {
                    ...
                    toast.show(qsTr("Better price found: %1. Updating forms.")
                                    .arg(parseFloat(newOrder.price).toFixed(8)),
                                Constants.General.time_toast_basic_info, "")
                    selectedOrder = newOrder
                }
                else
                {
                    console.log("Asking user if he want a better price but lower volume");
                    toast.show(qsTr("Better price (%1) found but received quantity (%2) is lower than your current one (%3).")
                                    .arg(parseFloat(newOrder.price).toFixed(8))
        ...
        Item
        {
            ...
            Connections {
                ...
                function onBestChanged() {
                    // Best-orders search filter (setFilterFixedString) is not
                    // available on this tree's proxy models; the list simply
                    // refreshes via refresh_best_orders().
                }
            }
    ...
    Row
    {
        ...
        DexIconButton
        {
            ...
            icon: "qrc:/assets/images/qaterial/repeat.svg"
            iconSize: 14
